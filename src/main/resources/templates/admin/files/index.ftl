...
<body class="gray-bg">
    ...
    <script id="operateHtml" type="text/html">
    		<button class="btn btn-primary btn-xs" type="button" onclick="edit('{{row.pid}}')">
    			<i class="fa fa-edit"></i>&nbsp;修改
    		</button> &nbsp;
            <button class="btn btn-danger btn-xs" type="button" onclick="del('{{row.pid}}')">
                <i class="fa fa-remove"></i>&nbsp;删除
            </button> &nbsp;
            <button class="btn btn-warning btn-xs" type="button" onclick="change('{{row.pid}}','status')">
                {{if row.status == 0 }}
            ...
            <button class="btn btn-info btn-xs" type="button" onclick="change('{{row.pid}}','carousel')">
                {{if row.carousel == 0 }}
                <i class="fa fa-user-secret"></i>&nbsp;不轮播
                {{else}}
                <i class="fa fa-user-secret"></i>&nbsp;轮播
                {{/if}}
            </button> &nbsp;
    </script>
    ...
    <script>
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
                    ...
                    formatter: function (value, row, index) {
                        return '<img style="width: 60px;height: 60px"  src="' + value + '" class="img-circle" >';
                    }
			    },{
                    title: "缩略图",
                    field: "thumb",
                    ...
                    formatter: function (value, row, index) {
                        if (value == '0')
                            return '<span class="label label-info">不轮播</span>';
                        else
                            return '<span class="label label-info">轮播</span>';
                    }
                },{
			        title: "操作",
			        field: "empty",
        ...
        function edit(id){
        	layer.open({
        	      ...
        	      shade: false,
                  area: ['80%', '75%'],
        	      content: '${ctx!}/admin/files/form?id=' + id,
        	      end: function(index){
